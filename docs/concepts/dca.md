...
**Trade Execution with Risk Management:**  
  ...
  A fixed amount is invested at set intervals, smoothing out the purchase price over multiple trades. By averaging out the purchase price over time, DCA helps to reduce the impact of short-term market volatility, making it an attractive strategy for long-term investors.

  The system handles multi-hop token swaps (if the swap path has more than two tokens) and ensures that enough funds are reserved for autonomous gas fees. A fee of 0.7% is deducted during the swap.
...
1. **Set Up Your DCA Order:**  
   ...
   - **Token In and Out:** The token use to pay for the DCA and the token that will be bought.
...
<p align="center">
  <img src="/img/dca_endTime.png" alt="DCA setting endtime" width="500px" />
   ...
   - **Slippage:** Set a slipppage value (superior of 1%) to manage acceptable price slippage or volatility. The price comparaison is made with the price oracle.
...
2. **Monitoring and Adjustments:**  
  By clicking on the DCA, a performance graph and history of previous swap is shown.
...
- Each execution of a DCA order verifies that the user has sufficient token balance and has approved the contract to spend the required amount. If these conditions are not met, the DCA order is cancelled.
